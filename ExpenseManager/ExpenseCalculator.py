...
class ZooKeeperExpenseCalculator:
    ...
    def extract_animal_compound_type(self):
        ...
        compound_wise_hours_of_care_per_week = self.animal_data.groupby('compound').sum().reset_index().sort_values('compound')
        # Return only the compound name and total hours required
        return compound_wise_hours_of_care_per_week[['compound', 'total_animal_care_in_hours_per_week']]
